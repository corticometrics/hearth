...
TEMPLATE_PATH = pkg_resources.resource_filename("hearth", "templates")
...
dcmqi_docker_image_help = (
    "Name of the dcmqi docker image " "(default: qiicr/dcmqi:latest)"
)
freesurfer_type_help = (
    "Use docker or local installed version of freesurfer " "(default: docker)"
)
...
t1_dicom_file_help = "Path to one of the T1-weighted DICOM files processed with FreeSurfer to create the aseg."
sr_template_help = (
    "Path to DICOM SR template that is filled in with aseg.stats values "
    "(default: provided within package)"
)
aseg_dicom_seg_file_help = (
    "DICOM SEG of the aseg, for example created by `hearth create-seg` "
    "(default: ./aseg.dcm)"
)
...
@click.option(
    "--dcmqi_docker_image", default="qiicr/dcmqi:latest", help=dcmqi_docker_image_help
)
@click.option(
    "--freesurfer_type",
    type=click.Choice(["docker", "local"]),
...
@click.command()
@click.argument("t1_dicom_file", type=click.Path(exists=True, resolve_path=True))
@click.argument("aseg_image_file", type=click.Path(exists=True, resolve_path=True))
@click.argument(
...
@click.pass_context
def create_seg(
    ctx, t1_dicom_file, aseg_image_file, aseg_dicom_seg_output, seg_metadata
):
    """
    ...
    """
    ctx = utils.check_requirements(ctx)
    ...
    with tempfile.TemporaryDirectory() as seg_temp_dir:
        print(
            "[hearth] Running create-seg with {temp_dir} as tempdir\n".format(
                temp_dir=seg_temp_dir
            )
        )
        resampled_aseg = os.path.join(seg_temp_dir, "aseg_native_space.nii.gz")
        t1_dicom_dir = utils.abs_dirname(t1_dicom_file)
        docker_user_string = utils.get_docker_user(aseg_image_file)

        resample_aseg_cmd = seg.get_resample_aseg_cmd(
            aseg_image_file, t1_dicom_file, resampled_aseg
        )
        generate_dicom_seg_cmd = seg.get_generate_dicom_seg_cmd(
            resampled_aseg, seg_metadata, t1_dicom_file, aseg_dicom_seg_output
        )
        ...
        if ctx.obj["freesurfer_type"] == "docker":
            ...
            """
            volumes_dict = {
                t1_dicom_dir: {"bind": t1_dicom_dir, "mode": "ro"},
                aseg_image_file: {"bind": aseg_image_file, "mode": "ro"},
                seg_temp_dir: {"bind": seg_temp_dir, "mode": "rw"},
            }

            environment_dict = {
                "FS_KEY": utils.base64_convert(ctx.obj["fs_license_key"])
            }

            utils.run_docker_commands(
                docker_image=ctx.obj["freesurfer_docker_image"],
        ...
        if ctx.obj["dcmqi_type"] == "docker":
            ...
            volumes_dict = {
                resampled_aseg: {"bind": resampled_aseg, "mode": "ro"},
                seg_metadata: {"bind": seg_metadata, "mode": "ro"},
                t1_dicom_dir: {"bind": t1_dicom_dir, "mode": "ro"},
                output_dir: {"bind": output_dir, "mode": "rw"},
            }

            utils.run_docker_commands(
                docker_image=ctx.obj["dcmqi_docker_image"],
...
@click.command()
@click.argument("t1_dicom_file", type=click.Path(exists=True, resolve_path=True))
@click.argument("aseg_stats_file", type=click.Path(exists=True, resolve_path=True))
@click.argument(
    "aseg_dicom_sr_output", default=os.path.join(os.getcwd(), "aseg-sr.dcm")
)
@click.option(
    "--aseg_dicom_seg_file",
    type=click.Path(exists=True, resolve_path=True),
...
def create_sr(
    ...
    generate_dicom_sr_cmd = sr.get_generate_dicom_sr_cmd(
        t1_dicom_file, aseg_dicom_seg_file, aseg_dicom_sr_output, sr_metadata_output
    )

    print("[hearth] Running create-sr\n")
    ...
    if ctx.obj["dcmqi_type"] == "docker":
        ...
        volumes_dict = {
            t1_dicom_dir: {"bind": t1_dicom_dir, "mode": "ro"},
            aseg_dicom_dir: {"bind": aseg_dicom_dir, "mode": "ro"},
            sr_metadata_output: {"bind": sr_metadata_output, "mode": "ro"},
            output_dir: {"bind": output_dir, "mode": "rw"},
        }
