...
def add_gm_wm_to_dataframe(aseg_dataframe, aseg_stats_file):
    ...
    label_name_dict = {
        "lhCerebralWhiteMatter": "Left-Cerebral-White-Matter",
        ...
        for line in f:
            for label in label_name_dict:
                if label in line:
                    ...
                    row = {
                        "SegId": label_number_dict[struct],
                        "NVoxels": np.nan,
                        "Volume_mm3": vol,
                        "StructName": struct,
                        "normMean": np.nan,
...
def generate_aseg_dicom_sr_metadata(
    dicom_sr_template,
    aseg_dicom_seg_file,
    t1_dicom_file,
    ...
    template_vars = {
        "aseg_dicom_seg_filename": aseg_dicom_seg_filename,
...
def get_generate_dicom_sr_cmd(
    t1_dicom_file, aseg_dicom_seg_file, aseg_dicom_sr_output, aseg_dicom_sr_metadata
):
    command_template = """\
tid1500writer \
